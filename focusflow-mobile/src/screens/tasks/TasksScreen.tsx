...
export default function TasksScreen() {
  ...
  const loadTasks = useCallback(async (pageNum = 0, reset = false) => {
    try {
      setLoading(true);
      const response = await taskService.getAll(pageNum, 20, 'createdAt', 'DESC');
  ...
  const handleSearch = async () => {
    try {
      setLoading(true);
      const response = await taskService.search({
  ...
  return (
    <View style={styles.container}>
      ...
      <FAB
        icon="plus"
        style={styles.fab}
        onPress={() => navigation.navigate('TaskDetail' as never, { taskId: null } as never)}
      />
    </View>
  );
}
